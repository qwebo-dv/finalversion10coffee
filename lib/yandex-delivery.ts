...
function getPositiveEnvironmentNumber(name: string, fallback: number) {
...
function getYandexDeliveryConfig(): YandexDeliveryConfig {
  ...
  return {
    ...
    // These values keep delivery available while managers fill the exact
    // dimensions in Payload. Exact product data always takes precedence.
    fallbackPackage: {
      lengthCm: getPositiveEnvironmentNumber("YANDEX_DELIVERY_PACKAGE_LENGTH_CM", isTest ? 30 : 45),
      widthCm: getPositiveEnvironmentNumber("YANDEX_DELIVERY_PACKAGE_WIDTH_CM", isTest ? 20 : 30),
      heightCm: getPositiveEnvironmentNumber("YANDEX_DELIVERY_PACKAGE_HEIGHT_CM", isTest ? 15 : 20),
      tareGrams: getPositiveEnvironmentNumber("YANDEX_DELIVERY_PACKAGE_TARE_GRAMS", 100),
      itemWeightGrams: getPositiveEnvironmentNumber("YANDEX_DELIVERY_DEFAULT_ITEM_WEIGHT_GRAMS", isTest ? 100 : 1000),
    },
  }
}

function getShippingDimension(value: number | null, fallback: number): number {
  const normalized = Number(value)
  return Number.isInteger(normalized) && normalized > 0 ? normalized : fallback
}

function normalizeShippingLines(lines: YandexDeliveryLine[], fallbackPackage: YandexDeliveryConfig["fallbackPackage"]) {
  return lines.map((line) => {
    const lengthCm = getShippingDimension(line.lengthCm, fallbackPackage.lengthCm)
    const widthCm = getShippingDimension(line.widthCm, fallbackPackage.widthCm)
    const heightCm = getShippingDimension(line.heightCm, fallbackPackage.heightCm)
    const weightGrams = getShippingDimension(line.weightGrams, fallbackPackage.itemWeightGrams)
    return { ...line, lengthCm, widthCm, heightCm, weightGrams }
  })
}

function getParcelDimensions(lines: ReturnType<typeof normalizeShippingLines>, tareGrams: number) {
  ...
  const weightGrams = tareGrams + lines.reduce((sum, line) => sum + line.weightGrams * Math.max(1, Math.floor(line.quantity)), 0)
...
export async function createYandexDeliveryOffer(params: {
  ...
  const config = getYandexDeliveryConfig()
  const shippingLines = normalizeShippingLines(params.lines, config.fallbackPackage)
  const parcel = getParcelDimensions(shippingLines, config.fallbackPackage.tareGrams)
