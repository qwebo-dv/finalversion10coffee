...
import { formatDeliveryDays } from "@/lib/utils/delivery-estimate"
...
type CdekPackage = { length: number; width: number; height: number; weight: number }
...
export function CdekDeliverySelector({
  ...
  onChange: (selection: ShopCdekSelection | null) => void
}) {
  ...
  const [packages, setPackages] = useState<CdekPackage[]>([])
  const [offices, setOffices] = useState<Office[]>([])
  const [office, setOffice] = useState<Office | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [courierAddress, setCourierAddress] = useState(defaultAddress)
  const [courierAddressComplete, setCourierAddressComplete] = useState(false)
  const [storageRestored, setStorageRestored] = useState(false)
  const searchTimer = useRef<ReturnType<typeof setTimeout> | null>(null)
  const restoredAddress = useRef("")
  const restoreCancelled = useRef(false)
  const activeTariff = type === "pickup" ? pickup : courierAddressComplete ? courier : null
  ...
  useEffect(() => {
    if (!city || !activeTariff || (type === "pickup" && !office) || (type === "courier" && !courierAddressComplete)) {
      onChange(null)
      return
    }
    onChange({ cityCode: city.code, cityName: city.city, deliveryType: type, office: office ? { code: office.code, name: office.name, address: office.address_full } : undefined, deliveryCost: activeTariff.price, minDays: activeTariff.minDays, maxDays: activeTariff.maxDays })
  }, [activeTariff, city, courierAddressComplete, office, onChange, type])
  ...
  return <div className="mt-5 space-y-4">
    ...
    {city && <><div><label className="mb-2 block text-xs font-bold text-[#655c55]">Тип доставки</label><div className="grid grid-cols-2 gap-3"><button type="button" onClick={() => setType("pickup")} className={`rounded-2xl border p-3 text-left text-sm font-bold ${type === "pickup" ? "border-[#5b328a] bg-[#f4edfa] text-[#5b328a]" : "border-black/10"}`}><Package className="mb-2 h-4 w-4" />Пункт выдачи{pickup && <span className="mt-1 block text-xs text-[#7d736b]">{formatDeliveryDays(pickup.minDays, pickup.maxDays)} · {formatPrice(pickup.price)}</span>}</button><button type="button" onClick={() => setType("courier")} className={`rounded-2xl border p-3 text-left text-sm font-bold ${type === "courier" ? "border-[#5b328a] bg-[#f4edfa] text-[#5b328a]" : "border-black/10"}`}><Truck className="mb-2 h-4 w-4" />Курьером{courierAddressComplete && courier && <span className="mt-1 block text-xs text-[#7d736b]">{formatDeliveryDays(courier.minDays, courier.maxDays)} · {formatPrice(courier.price)}</span>}</button></div></div>{loading && <p className="flex items-center gap-2 text-sm text-[#7d736b]"><Loader2 className="h-4 w-4 animate-spin" />Рассчитываем стоимость доставки…</p>}{error && <p className="text-sm text-red-700">{error}</p>}{activeTariff && !loading && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><div className="flex justify-between gap-3"><b>{activeTariff.name}</b><b>{formatPrice(activeTariff.price)}</b></div><p className="mt-1">Ориентировочный срок: {formatDeliveryDays(activeTariff.minDays, activeTariff.maxDays)}</p></div>}{type === "pickup" && <div className="space-y-3"><label className="block text-xs font-bold text-[#655c55]">Пункт выдачи</label>{pickup && <CdekPickupMap cityName={city.city} offices={offices} selectedOfficeCode={office?.code} tariff={pickup} packages={packages} weightGrams={weightGrams} onSelect={setOffice} />} {office && <div className="rounded-2xl border border-[#5b328a]/20 bg-[#f4edfa] p-4 text-sm text-[#5b328a]"><b className="block">Выбран ПВЗ: {office.name}</b><span className="mt-1 block text-xs">{office.address_full}</span></div>}<details className="rounded-2xl border border-black/10 bg-white"><summary className="cursor-pointer px-4 py-3 text-sm font-bold text-[#655c55]">Выбрать из списка ({offices.length})</summary><div className="max-h-56 space-y-1 overflow-y-auto border-t border-black/10 p-1">{offices.map((item) => <button key={item.code} type="button" onClick={() => setOffice(item)} className={`block w-full rounded-xl p-3 text-left text-sm ${office?.code === item.code ? "bg-[#f4edfa] text-[#5b328a]" : "hover:bg-[#f8f5f1]"}`}><b>{item.name}</b><span className="mt-1 block text-xs text-[#7d736b]">{item.address_full}</span></button>)}{offices.length === 0 && <p className="p-3 text-sm text-[#7d736b]">Пункты выдачи появятся после выбора города.</p>}</div></details></div>}{type === "courier" && <label className="block"><span className="mb-2 block text-xs font-bold text-[#655c55]">Улица, дом, квартира</span><AddressInput name="address" required requireHouse value={courierAddress} onChange={setCourierAddress} onCompleteChange={setCourierAddressComplete} city={city.city} className="h-12 rounded-2xl border-black/10 px-4 focus-visible:border-[#5b328a] focus-visible:ring-0" placeholder="Начните вводить улицу и дом" /></label>}</>}
  </div>
}
