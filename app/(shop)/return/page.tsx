...
export default async function ReturnPage() {
  ...
  ].map(([title, text]) => <section key={title} className="rounded-[28px] border border-black/[0.07] bg-white p-7 sm:p-9"><h2 className="text-2xl font-black">{title}</h2><p className="mt-4 text-sm leading-7 text-[#554b43]">{text}</p></section>)}</div><div className="mt-10 rounded-[28px] bg-[#5b328a] p-8 text-white"><h2 className="text-2xl font-black">Как обратиться</h2><p className="mt-3 text-sm leading-7 text-white/75">Напишите на <a className="font-black text-white" href="mailto:info@10coffee.ru">info@10coffee.ru</a> или позвоните по номеру <a className="font-black text-white" href="tel:+79384537060">+7 (938) 453-70-60</a>. Укажите номер заказа, товар и причину обращения.</p></div></article></main>
}
